...
class ClientSearchField extends StatefulWidget {
  const ClientSearchField(
      {super.key, required this.title, required this.data, required this.type,required this.controller});
  final String title;
  final List data;
  final String type;
...
}: widget.type=='comp_product'? (p0) {
  CompProductModel compProductModel = p0.item as CompProductModel;
...
recentDriverID=car.id;
}:  null,     controller: widget.controller,
        ...
        suggestions: suggestions
            .map((e) => widget.type == 'client'
                ...
                :  widget.type == 'variables'? SearchFieldListItem<VariablesModel>(e.text,
                    child: ListTile(
                     
                     title:     Text(e.text),
                        
                      
                    )):   widget.type=='comp_product'? SearchFieldListItem<CompProductModel>(e.name,item: e,
                    child: ListTile(
                      subtitle: Column(
                        crossAxisAlignment: CrossAxisAlignment.end,
                        children: [
                ...
                Text(e.price.toString()),
                    ...
                    )):widget.type=='car'? SearchFieldListItem<CarModel>(e.car,item: e,
                    child: ListTile(
                      subtitle: Column(
                        crossAxisAlignment: CrossAxisAlignment.end,
                        children: [
                    ...
                    )):SearchFieldListItem<ProductModel>(e.name,item: e,
                    child: ListTile(
                      subtitle: Column(
                        crossAxisAlignment: CrossAxisAlignment.end,
                        children: [
...
class UserModel {
  ...
  CarModel({
    required this.car,required this.name,required this.id
  });
...
class VariablesModel {
  ...
  VariablesModel({
    required this.category,required this.text
  });
