...
Future<void> varsChecker() async {
  ...
  final existingDocumentRef = FirebaseFirestore.instance.collection('vars').doc('id');


  // Retrieve the field value from the existing document
  final documentSnapshot = await existingDocumentRef.get();
  if (documentSnapshot.exists) {
    final fieldValue = documentSnapshot.data()!['visits']; // Replace 'fieldToRetrieve' with the actual field name
  ...
  void createClient({required LatLng position,required String address,city,government,placeMark}) {
    _store
        .collection('clients')
        .add(ClientModel(
  ...
  getIds() async {
    var ids =
         await _store.collection('vars').doc('id').get();
        return ids;
  }
}
